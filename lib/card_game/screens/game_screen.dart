...
class _GameScreenState extends State<GameScreen> {
  ...
  Widget _buildBiddingPlayerHand(GameController controller) {
    ...
    final compact = screenHeight < 700;
    final cardWidthByScreen = isPortrait ? (screenWidth - 32) / 6 - 4 : (screenHeight * 0.1).clamp(55.0, 100.0);
      ...
      if (attackTrickWins == 10) {
        ...
        if (attackTrickWins == 9 && attackWins) {
          ...
          if (lostTricks.length == 1) {
            ...
            final isAttackLed = isAttack(lost.leadPlayerId);
  ...
  String? _describeLeadFromIntent(Trick trick, GameState state, AppLocalizations l10n) {
    ...
    switch (intent) {
      ...
      case LeadIntent.defenseLowCard:
        ...
        final dlcLeadAttack = trick.leadPlayerId != null && (trick.leadPlayerId == state.declarerId || trick.leadPlayerId == state.friendId);
        if (dlcLeadAttack) {
          final attackWon = trick.winnerId != null && (trick.winnerId == state.declarerId || trick.winnerId == state.friendId);
      ...
      child: Column(
        ...
        children: [
          ...
          if (state.declarerId != null) ...[
            ...
            // 점수 획득 전략
            _buildStrategySection(controller),
            const SizedBox(height: 10),
          ],
          const SizedBox(height: 16),
          Center(
  ...
  Widget _buildStrategySection(GameController controller) {
    final l10n = getL10n(context);
                        ...
                        children: [
                          ...
                          if (playerRoles[pid] != null)
                            Text(
                              ...
                              style: TextStyle(
                                fontSize: fontSize - 2,
                                color: state.players[pid].isDeclarer
                                    ? (darkTheme ? Colors.red[300] : Colors.red[600])
                ...
                TableRow(
                  children: [
                    ...
                    for (final pid in displayOrder)
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 6),
                        color: row.winnerId == pid ? winnerBg : null,
